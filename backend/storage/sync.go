...
func (w *Watcher) syncEvent(event AlbumEvent, ctx context.Context) {
	tx, err := w.db.Begin()
	if err != nil {
		fmt.Printf("syncInRealTime: failed to start db transaction: %v\n", err)
		return
	}
	switch event.Type {
	case "add":
		...
		if err != nil {
			fmt.Printf("syncInRealTime: failed to create new album: %v\n", err)
			return
		}
		if err := w.db.saveAlbum(tx, album); err != nil {
			tx.Rollback()
			fmt.Printf("syncInRealTime: failed to save album: %v\n", err)
			return
		}
		if err := tx.Commit(); err != nil {
			fmt.Printf("syncInRealTime: failed to commit db transaction: %v\n", err)
			return
		}
	...
	case "remove":
		if err := w.db.RemoveAlbumByPath(event.Path); err != nil {
			tx.Rollback()
			fmt.Printf("syncInRealTime: failed to remove album: %v\n", err)
			return
		}
		if err := tx.Commit(); err != nil {
			fmt.Printf("syncInRealTime: failed to commit db transaction: %v\n", err)
